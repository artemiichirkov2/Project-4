    ...
    private static int AddQuizFromFile(Scanner scanner, Course thisCourse, int courseIndex) throws IOException {
		...
		try {
			...
			while (line != null) {
                ...
                if (lines[1].equals(quizName)) {
                    System.out.println("This quiz already exists, try again");
                }
			}
			reader.close();
		} catch (IOException e) {
        ...
        String finalQuiz = quizName + "\n";

        System.out.println("Enter the amount of questions you want to have in this quiz");
        
        amountOfQuestions = Integer.parseInt(scanner.nextLine());
        
        for (int i = 0;i < amountOfQuestions;i++) {
            int f = i + 1;
            System.out.println("Enter the " + f + " question");
            question = scanner.nextLine();
    
            String finalQuestion = question + "\n";
    
            System.out.println("How many answer choices do you want" + 
                " this question to have?");
            amountOfChoices = Integer.parseInt(scanner.nextLine());
    
            for (int j = 0;j < amountOfChoices;j++) {
                int k = j + 1;
                System.out.println("Enter the " + k + " choice");
                answerChoice = scanner.nextLine();
    
                finalQuestion += k + ") " + answerChoice + " ";
            }
    
            finalQuiz += finalQuestion;
        }
    
        File file = new File(quizName + ".txt");
